...
namespace nanolog
{
	...
	private:
		void init_fileset()
		{
			for (auto& p : fs::directory_iterator(m_log_directory))
			{
				const fs::path& path = p.path();
				insert(path.filename().string());
			}
		}
		...
		void create_file()
		{
			if (m_first_create) {
				...
				std::string file_path = m_name + file_data_to_filename(cur_file_data);
			...
			else
			{
				...
				std::string file_path = m_name + logfile_suffix;
				m_os.reset(new std::ofstream());
				m_os->open(file_path, std::ofstream::out | std::ofstream::app);
		...
		void delete_exceed_file() {
			if (m_filenum > 0 && m_file_value_set.size() > m_filenum)
			{
				...
				while (it_set != m_file_value_set.end())
				{
					if (num >= m_filenum)
					{
						std::string filename = m_name + file_data_to_filename(*it_set);
						try {
							fs::remove(filename);
						}
	...
	std::atomic < unsigned int > g_loglevel;
	std::unique_ptr < NanoLogger > g_nanologger;
	std::atomic < NanoLogger* > g_atomic_nanologger;
	...
	public:
		...
		*/
		static void initialize(GuaranteedLogger gl, std::string const& log_directory, std::string const& log_file_name, uint32_t log_file_roll_size_mb, int file_num)
		{
			...
			g_nanologger.reset(new NanoLogger(gl, log_directory, log_file_name, log_file_roll_size_mb, file_num));
			g_atomic_nanologger.store(g_nanologger.get(), std::memory_order_seq_cst);
		}
		...
		{
			g_nanologger.reset(new NanoLogger(ngl, log_directory, log_file_name, log_file_roll_size_mb, file_num));
			g_atomic_nanologger.store(g_nanologger.get(), std::memory_order_seq_cst);
		}
	...
	public:
		...
		{
			g_atomic_nanologger.load(std::memory_order_acquire)->add(std::move(logline));
			return true;
		}
	};
