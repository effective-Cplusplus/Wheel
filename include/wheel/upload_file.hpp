#ifndef upload_file_h__
#define upload_file_h__
	...
	namespace http_servers {
		...
		public:
			...
			bool remove()
			{
				...
				bool flag = fs::remove(fs::path(file_path_.c_str()),ec);
				file_path_ = "";
				file_size_ = 0;
				return flag;
			}
			...
			bool copy_to(const std::string& directory_path, const std::string& file_name = "") const
			{
				...
				fs::copy_file(fs::path(file_path_.c_str()), fs::path(write_directory_path + write_file_name));
				return true;
			}
		...
		private:
			void check_and_create_directory(const std::string& direcotry_path) const
			{
				...
				wheel::unit::split(vec,std::string(direcotry_path.data(), direcotry_path.size()), "/");
