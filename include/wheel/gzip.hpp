...
#define GZIP_ENCODING 16
	...
	// @return - true on success, false on failure
		static bool compress(const std::string& data, std::string& compressed_data, int level = -1) {
		...
		// @return - true on success, false on failure
		static bool uncompress(const std::string& compressed_data, std::string& data) {
			int ret;
			unsigned have;
			z_stream strm;
			unsigned char out[CHUNK] = { 0 };
		...
		static int compress_file(const char* src_file, const char* out_file_name)
		{
			char buf[BUFSIZ] = { 0 };
			uInt bytes_read = 0;
		...
		static int uncompress_file(const char* src_file, const char* out_file_name) {
			char buf[BUFSIZ] = { 0 };
