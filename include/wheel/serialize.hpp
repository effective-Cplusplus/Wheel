...
namespace wheel {
	namespace serialization {
		...
		std::enable_if_t<std::is_floating_point<T>::value> render_json_value(Stream& ss, T value){
			std::string temp;
			temp.resize(1024);
			sprintf(&temp[0], "%f", value);
			int len = strlen(temp.c_str());
			ss.write(temp.c_str(), len);
		}
		...
		template<typename Stream, typename T>
		constexpr auto to_json(Stream& ss, T&& t)->std::enable_if_t<reflector::is_reflection<T>::value>;
		...
		template<typename Stream, typename T>
		std::enable_if_t <traits::is_sequence_container<T>::value> render_json_value(Stream& ss, const T& v) {
			ss.put('[');
		...
		auto write_json_key = [](auto& s, auto i, auto& t) {
			s.put('"');
			auto name = reflector::get_name<decltype(t), decltype(i)::value>(); //will be replaced by string_view later
			s.write(name.data(), name.length());
			s.put('"');
		};

		template<typename Stream, typename T>
		constexpr auto to_json(Stream& s, T&& v)->std::enable_if_t<traits::is_sequence_container<std::decay_t<T>>::value>{
		...
		template<typename Stream, typename T>
		constexpr auto to_json(Stream& s, T&& t) -> std::enable_if_t<reflector::is_reflection<T>::value>{
		...
		static void skip(reader_t& rd);

		static void skip_array(reader_t& rd) {
			auto tok = &rd.peek();
			while (tok->str.str[0] != ']') {
				skip(rd);
				tok = &rd.peek();
		...
		static void skip_key(reader_t& rd) {
			auto& tok = rd.peek();
			switch (tok.type) {
			case token::t_string:
			case token::t_int:
		...
		static void skip_object(reader_t& rd) {
			auto tok = &rd.peek();
			while (tok->str.str[0] != '}') {
				skip_key(rd);
				tok = &rd.peek();
		...
		template<typename T>
		static void check_result(T val, char const* str) {
			if (*str == 'n')
				return;
		...
		//read json to value
		template<typename T>
		static std::enable_if_t<traits::is_signed_intergral_like<T>::value> read_json(reader_t& rd, T& val) {
			auto& tok = rd.peek();
			switch (tok.type) {
			case token::t_string: {
		...
		template<typename T>
		static std::enable_if_t<traits::is_unsigned_intergral_like<T>::value> read_json(reader_t& rd, T& val) {
			auto& tok = rd.peek();
			switch (tok.type) {
			case token::t_string: {
		...
		template<typename T>
		static std::enable_if_t<std::is_enum<T>::value> read_json(reader_t& rd, T& val) {
			typedef typename std::underlying_type<T>::type RAW_TYPE;
			read_json(rd, (RAW_TYPE&)val);
		}

		template<typename T>
		static std::enable_if_t<std::is_floating_point<T>::value> read_json(reader_t& rd, T& val){
			auto& tok = rd.peek();
			switch (tok.type){
			case token::t_string:{
...
#define MIN_NUMBER_VALUE 1e-8
		static void read_json(reader_t& rd, bool& val){
			auto& tok = rd.peek();
			switch (tok.type){
			case token::t_string:{
		...
		static void read_json(reader_t& rd, std::string& val) {
			auto& tok = rd.peek();
		...
		static void read_json(reader_t& rd, T(&val)[N]){
			read_array(rd, val);
		}

		template<typename T>
		static void read_array(reader_t& rd, T& val)
		{
			if (rd.expect('[') == false) {
		...
		template <typename T, size_t N>
		static void read_json(reader_t& rd, std::array<T, N>& val){
			read_array(rd, val);
		}

		template<typename T>
		static std::enable_if_t<traits::is_emplace_back_able<T>::value> emplace_back(T& val) {
			val.emplace_back();
		}

		template<typename T>
		static std::enable_if_t<traits::is_sequence_container<T>::value> read_json(reader_t& rd, T& val) {
			if (rd.expect('[') == false) {
				rd.error("array must start with [.");
				return;
			}
		...
		template<typename T>
		static std::enable_if_t<traits::is_associat_container<T>::value> read_json(reader_t& rd, T& val) {
			if (rd.expect('{') == false){
		...
		inline constexpr std::enable_if_t<traits::is_tuple<std::decay_t<T>>::value, bool>
			...
			reader_t rd(buf, len);
			rd.next();
			reflector::for_each_tuple_front(t, [&rd](auto& v, auto i){
		...
		template<typename T>
		inline constexpr std::enable_if_t<traits::is_sequence_container<std::decay_t<T>>::value, bool>
